...
const ShiftManagement = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${import.meta.env.VITE_APP_API_URL}/api/employee-records`, {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token') // Get the token from localStorage
    if (token) {
      fetch(`${import.meta.env.VITE_APP_API_URL}/api/shifts`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`, // Send the token in the Authorization header
  ...
  const submitShiftDetails = () => {
    ...
    if (token) {
      fetch(`${import.meta.env.VITE_APP_API_URL}/api/shift`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
